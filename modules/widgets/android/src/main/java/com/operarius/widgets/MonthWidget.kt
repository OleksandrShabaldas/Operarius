...
class MonthWidget : AppWidgetProvider() {
  ...
  override fun onDeleted(ctx: Context, ids: IntArray) {
    ...
    for (id in ids) e.remove(offsetKey(id))
    e.apply()
  }
  ...
  companion object {
    private const val ACTION_PREV = "com.operarius.widgets.MONTH_PREV"
    private const val ACTION_NEXT = "com.operarius.widgets.MONTH_NEXT"
    private const val ACTION_TODAY = "com.operarius.widgets.MONTH_TODAY"
    private const val DARK = 0xFF0B0B0D.toInt()

    private fun offsetKey(id: Int) = "month_offset_$id"

    private fun action(ctx: Context, id: Int, action: String, code: Int): PendingIntent =
      PendingIntent.getBroadcast(
        ctx,
        id * 8 + code,
        Intent(ctx, MonthWidget::class.java).setAction(action).putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, id),
        PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE,
      )

    // How many task lines fit in a day of this size (and how many when a "+N" line needs room too).
    private fun fit(ctx: Context, mgr: AppWidgetManager, id: Int, rows: Int): Pair<Int, Int> {
      ...
      val grid = h - 20f - 36f * scale - 16f * scale // padding, header, weekdays
    ...
    fun render(ctx: Context, mgr: AppWidgetManager, id: Int) {
      ...
      val (fitAll, fitMore) = fit(ctx, mgr, id, rows)
      ...
      rv.setOnClickPendingIntent(R.id.m_today, action(ctx, id, ACTION_TODAY, 0))
      rv.setOnClickPendingIntent(R.id.m_prev, action(ctx, id, ACTION_PREV, 1))
      rv.setOnClickPendingIntent(R.id.m_next, action(ctx, id, ACTION_NEXT, 2))
      rv.setOnClickPendingIntent(R.id.m_title, if (off != 0) action(ctx, id, ACTION_TODAY, 3) else Refresh.open(ctx, "operarius://day?date=$todayKey", id * 64 + 63))
